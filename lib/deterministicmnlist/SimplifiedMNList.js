...
function SimplifiedMNList(simplifiedMNListDiff) {
   ...
   * ordered according to their creation time
   * @type {QuorumEntry[]}
   */
  this.validMNs = [];
 ...
 */
SimplifiedMNList.prototype.addOrUpdateQuorums = function addOrUpdateQuorums(quorumEntries) {
  ...
  var newQuorumsByType = llmqTypeArray.map((type) => {
    var hits = newQuorumEntries.filter(quorum => quorum.llmqType === type);
    return { llmqType: type, quorums: hits }
  });

  var oldQuorumsByType = llmqTypeArray.map((type) => {
  ...
  newQuorumsByType.forEach(function (quorumsByType) {
    quorumsByType.quorums.forEach(function (quorum) {
      oldQuorumsByType[i].quorums.shift();
      oldQuorumsByType[i].quorums.push(quorum);
    });
    i = i + 1;
  }, this);
  this.quorumList = [];
  oldQuorumsByType.forEach((quorumsByType) => {
