...
describe('SimplifiedMNListStore', function () {
    ...
    describe('constructor', function () {
        it(`Should create an SMLStore with a diff of at least ${
            ...
            const firstBlockHash =
                SMNListFixture.getChainlockDiff0().baseBlockHash
    ...
    describe('add diffs', function () {
        it('add one diff to base', function () {
            ...
            const cbTx = new Transaction(
                SMNListFixture.getChainlockDiff16().cbTx
            )
            const baseHeight = cbTx.extraPayload.height
            expect(tipHeight).to.equal(baseHeight)
            expect(tipHash).to.equal(
                SMNListFixture.getChainlockDiff16().blockHash
            )
        })
        it('add two diff to base', function () {
            ...
            const tipHash = smlStore.getTipHash()
            const cbTx = new Transaction(
                SMNListFixture.getChainlockDiff17().cbTx
            )
            const baseHeight = cbTx.extraPayload.height
            expect(tipHeight).to.equal(baseHeight)
            expect(tipHash).to.equal(
                SMNListFixture.getChainlockDiff17().blockHash
            )
        })
        it('add three diff to base', function () {
            ...
            const tipHash = smlStore.getTipHash()
            const cbTx = new Transaction(
                SMNListFixture.getChainlockDiff18().cbTx
            )
            const baseHeight = cbTx.extraPayload.height
            expect(tipHeight).to.equal(baseHeight)
            expect(tipHash).to.equal(
                SMNListFixture.getChainlockDiff18().blockHash
            )
        })
    ...
    describe('get SML', function () {
        ...
        it('Should get the current SML', function () {
            ...
            expect(currentSML.merkleRootMNList).to.be.equal(
                currentMerkleRootMNList
            )
        })
