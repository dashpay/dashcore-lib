...
describe('ECDSA', function () {
    ...
    describe('#calci', function () {
        ...
        it('calulates this known i', function () {
            ...
            var ecdsa = new ECDSA({
                privkey: new Privkey(
                    BN.fromBuffer(Hash.sha256(Buffer.from('test')))
                ),
                hashbuf: hashbuf,
                sig: new Signature({
                    r: r,
    ...
    describe('#sign', function () {
        ...
        it('should generate right K', function () {
            ...
            var signature1 = ECDSA.sign(msg1, Privkey.fromBuffer(pk))
                .toBuffer()
                .toString('hex')
    ...
    describe('signing and verification', function () {
        ...
        describe('#verify', function () {
            ...
            it('should verify a valid signature, and unverify an invalid signature', function () {
                var sig = ECDSA.sign(ecdsa.hashbuf, ecdsa.privkey)
                ECDSA.verify(ecdsa.hashbuf, sig, ecdsa.pubkey).should.equal(
                    true
                )
                var fakesig = new Signature(sig.r.add(new BN(1)), sig.s)
                ECDSA.verify(ecdsa.hashbuf, fakesig, ecdsa.pubkey).should.equal(
                    false
                )
            })
        ...
        describe('vectors', function () {
            vectors.valid.forEach(function (obj, i) {
                it('should validate valid vector ' + i, function () {
                    var ecdsa = ECDSA().set({
                        privkey: new Privkey(
                            BN.fromBuffer(Buffer.from(obj.d, 'hex'))
                        ),
            ...
            vectors.invalid.sigError.forEach(function (obj, i) {
                it(
                    'should validate invalid.sigError vector ' +
                        i +
                        ': ' +
                        obj.description,
                    function () {
            ...
            vectors.deterministicK.forEach(function (obj, i) {
                it('should validate deterministicK vector ' + i, function () {
                    ...
                    var privkey = Privkey(
                        BN.fromBuffer(Buffer.from(obj.privkey, 'hex')),
                        'mainnet'
                    )
                    var ecdsa = ECDSA({
                        privkey: privkey,
