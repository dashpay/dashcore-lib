...
const bls = {
  ...
  async verifySignature(signatureHex, messageHash, publicKeyHex) {
    ...
    try {
      ...
      publicKey = blsInstance.PublicKey.fromBytes(Uint8Array.from(Buffer.from(publicKeyHex, 'hex')));
      aggregationInfo = blsInstance.AggregationInfo.fromMsgHash(publicKey, messageHash);
  ...
  async aggregatePublicKey(publicKeys, signersBits) {
    ...
    const aggregatedKey = blsInstance.PublicKey.aggregate(pks);
