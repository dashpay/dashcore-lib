 ...
 * @returns {boolean}
 */
ChainLock.prototype.verifySignatureAgainstQuorum = function verifySignatureAgainstQuorum(quorum) {
  return new Promise(resolve => {
    var quorumPublicKey = quorum.quorumPublicKey;
    var signature = this.signature;
    var signatureId = this.getSignatureIDForQuorumEntry(quorum);

    var blsInstance = bls.getInstance();
    return blsInstance.then((instance) => {

      var publicKey = instance.PublicKey.fromBytes(Buffer.from(quorumPublicKey, 'hex'))
      var insecureSignature = instance.InsecureSignature.fromBytes(signature);
      var isValid = insecureSignature.verify([signatureId], [publicKey]);
...
ChainLock.prototype.getRequestId = function getRequestId() {
  ...
  var prefix = ChainLock.Constants.CLSIG_REQUESTID_PREFIX;
...
ChainLock.prototype.getSignatureIDForQuorumEntry = function (quorum) {
  var llmqType = quorum.llmqType;
  var quorumHash = quorum.quorumHash;
