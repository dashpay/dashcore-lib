...
SubTxTransitionPayload.fromBuffer = function (rawPayload) {
  ...
  payload
    .setRegTxId(payloadBufferReader.read(HASH_SIZE).reverse().toString("hex"))
    .setHashPrevSubTx(
      payloadBufferReader.read(HASH_SIZE).reverse().toString("hex")
    )
  ...
  if (signatureSize > 0) {
    payload.payloadSigSize = signatureSize;
...
SubTxTransitionPayload.validatePayloadJSON = function (blockchainUserPayload) {
  ...
  Preconditions.checkArgumentType(
    blockchainUserPayload.version,
    "number",
    "version"
  );
  Preconditions.checkArgumentType(
    blockchainUserPayload.creditFee,
    "number",
    "creditFee"
  );

  Preconditions.checkArgument(
    isUnsignedInteger(blockchainUserPayload.version),
    "Expect version to be an unsigned integer"
  );
  Preconditions.checkArgument(
    isUnsignedInteger(blockchainUserPayload.creditFee),
    "Expect creditFee to be an unsigned integer"
  );

  Preconditions.checkArgument(
    isHexString(blockchainUserPayload.regTxId),
    "expect regTxId to be a hex string but got " +
      typeof blockchainUserPayload.regTxId
  );
  Preconditions.checkArgument(
    blockchainUserPayload.regTxId.length === constants.SHA256_HASH_SIZE * 2,
    "Invalid regTxId size"
  );

  Preconditions.checkArgument(
    isHexString(blockchainUserPayload.hashPrevSubTx),
    "expect hashPrevSubTx to be a hex string but got " +
      typeof blockchainUserPayload.hashPrevSubTx
  );
  Preconditions.checkArgument(
    blockchainUserPayload.hashPrevSubTx.length ===
      constants.SHA256_HASH_SIZE * 2,
    "Invalid hashPrevSubTx size"
  );

  Preconditions.checkArgument(
    isHexString(blockchainUserPayload.hashSTPacket),
    "expect hashSTPacket to be a hex string but got " +
      typeof blockchainUserPayload.hashSTPacket
  );
  Preconditions.checkArgument(
    blockchainUserPayload.hashSTPacket.length ===
      constants.SHA256_HASH_SIZE * 2,
    "Invalid hashSTPacket size"
  );

  if (blockchainUserPayload.payloadSig) {
    Preconditions.checkArgument(
      isHexString(blockchainUserPayload.payloadSig),
      "expect payloadSig to be a hex string but got " +
        typeof blockchainUserPayload.payloadSig
    );
    Preconditions.checkArgument(
      blockchainUserPayload.payloadSig.length ===
        constants.COMPACT_SIGNATURE_SIZE * 2,
