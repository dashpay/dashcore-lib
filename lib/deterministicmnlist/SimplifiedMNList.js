...
SimplifiedMNList.prototype.addOrUpdateQuorums = function addMNs(quorumEntries) {
  ...
  newQuorumEntries.forEach(function (newQuorumListEntry) {
    var indexOfOldEntry = this.quorumList.findIndex(function (oldQuorumListEntry) {
      return oldQuorumListEntry.quorumHash === newQuorumListEntry.quorumHash;
    });
    if (indexOfOldEntry > -1) {
...
SimplifiedMNList.prototype.calculateMerkleRootQuorums = function calculateMerkleRootQuorums() {
  ...
  var commitmentHashes = this.quorumList.map(
    function (quorumEntry) {
      return quorumEntry.getCommitmentHash();
    }
  );
  //console.log('commitmentHashes', commitmentHashes);
  //console.log('commitmentHashes len', commitmentHashes.length);
  commitmentHashes.sort(function (a, b) {
    return Buffer.compare(a, b);
  });
  //console.log('sorted commitmentHashes', commitmentHashes);
  return getMerkleRoot(getMerkleTree(commitmentHashes)).reverse().toString('hex');
};
