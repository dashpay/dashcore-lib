...
SubTxRegisterPayload.fromBuffer = function fromBuffer(rawPayload) {
  ...
  blockchainUserPayload.userName = payloadBufferReader
    .read(usernameLen)
    .toString();
  ...
  if (signatureSize > 0) {
    ...
    blockchainUserPayload.payloadSig = payloadBufferReader
      .read(signatureSize)
      .toString("hex");
  }
...
SubTxRegisterPayload.validatePayloadJSON = function (blockchainUserPayload) {
  ...
  Preconditions.checkArgument(
    BufferUtil.isBuffer(blockchainUserPayload.pubKeyId),
    "expect pubKeyId to be a Buffer but got " +
      typeof blockchainUserPayload.pubKeyId
  );
  Preconditions.checkArgument(
    blockchainUserPayload.pubKeyId.length === constants.PUBKEY_ID_SIZE,
    "Invalid pubKeyId size"
  );
  Preconditions.checkArgumentType(
    blockchainUserPayload.userName,
    "string",
    "userName"
  );
  Preconditions.checkArgument(
    blockchainUserPayload.userName.length > 1,
 ...
 * @return {SubTxRegisterPayload}
 */
SubTxRegisterPayload.prototype.setPubKeyIdFromPrivateKey = function (
  privateKey
) {
  this.setPubKeyId(
    SubTxRegisterPayload.convertPrivateKeyToPubKeyId(privateKey)
  );
  return this;
};
