...
function SimplifiedMNListStore(diffArray, options) {
  const firstDiff = new SimplifiedMNListDiff(diffArray[0]);
  ...
  // we only add to diffStore if more than one element is in diffArray
...
SimplifiedMNListStore.prototype.addDiff = function addDiff(diff) {
  ...
  if (this.diffStore.length >= this.maxDiffs - 1) {
    this.baseSimplifiedMNList = this.applyDiffs([this.diffStore[0].diff]);
