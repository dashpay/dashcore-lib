...
SimplifiedMNList.prototype.addOrUpdateMNs = function addMNs(mnListEntries) {
  ...
  newMNListEntries.forEach(function (newMNListEntry) {
    const indexOfOldEntry = this.mnList.findIndex(oldMNListEntry => oldMNListEntry.proRegTxHash === newMNListEntry.proRegTxHash);
...
SimplifiedMNList.prototype.addAndMaybeRemoveQuorums = function addAndMaybeRemoveQuorums(quorumEntries) {
  const llmqTypeArray = [...this.getLLMQTypes()];

  const newQuorumsByType = llmqTypeArray.map((type) => {
    const hits = quorumEntries.filter(quorum => quorum.llmqType === type);
    return { llmqType: type, quorums: hits };
  });

  const quorumListByType = llmqTypeArray.map((type) => {
    const hits = this.quorumList.filter(quorum => quorum.llmqType === type);
    return { llmqType: type, quorums: hits };
  });
  let i = 0;
...
SimplifiedMNList.prototype.sortQuorums = function sortQuorumsEntries(quorumList) {
  quorumList.sort((a, b) => Buffer.compare(Buffer.from(a.calculateHash()).reverse(), Buffer.from(b.calculateHash()).reverse()));
  return quorumList;
};
...
SimplifiedMNList.prototype.calculateMerkleRootQuorums = function calculateMerkleRootQuorums() {
  ...
  const sortedQuorums = this.sortQuorums(this.quorumList.slice());
  const sortedHashes = sortedQuorums.map(quorum => quorum.calculateHash().reverse());
 ...
 * it calculates sha256(sha256(proTxHash, confirmedHash), modifier) per MN
 * Please note that this is not a double-sha256 but a single-sha256
 * @param {llmqType} llmqType
 * @param {Buffer} modifier
 * @return {Object[]} scores
 */
SimplifiedMNList.prototype.calculateSignatoryQuorumScores = function calculateSignatoryQuorumScores(llmqType, modifier) {
  return this.getVerifiedQuorumsOfType(llmqType).map((quorum, index) => {
    const bufferWriter = new BufferWriter();
    bufferWriter.writeUInt8(llmqType);
    bufferWriter.writeReverse(quorum.quorumHash);
    bufferWriter.writeReverse(modifier);
    return { score: Hash.sha256(bufferWriter.toBuffer()).reverse(), index };
  });
};
