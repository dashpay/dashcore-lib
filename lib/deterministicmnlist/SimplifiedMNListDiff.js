 ...
 * @property {Array<string>} deletedQuorums - sha256 hashes of deleted quorums
...
SimplifiedMNListDiff.fromBuffer = function fromBuffer(buffer, network) {
  ...
  for (var i = 0; i < deletedQuorumsCount; i++) {
    data.deletedQuorums.push(bufferReader.read(constants.SHA256_HASH_SIZE).reverse());
  }
  ...
  for (var i = 0; i < newQuorumsSize; i++) {
    ...
    data.newQuorums.push(QuorumEntry.fromBuffer(bufferReader.read(quorumEntrySize)));
  }
...
SimplifiedMNListDiff.prototype.toBuffer = function toBuffer() {
  ...
  this.deletedQuorums.forEach(function (deletedQuorumHash) {
    bufferWriter.write(Buffer.from(deletedQuorumHash.quorumHash, 'hex'));
  });
