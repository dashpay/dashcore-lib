...
SubTxResetKeyPayload.fromBuffer = function (rawPayload) {
  ...
  payload.regTxHash = payloadBufferReader
    .read(HASH_SIZE)
    .reverse()
    .toString("hex");
  payload.hashPrevSubTx = payloadBufferReader
    .read(HASH_SIZE)
    .reverse()
    .toString("hex");
...
SubTxResetKeyPayload.prototype.validate = function () {
  ...
  Preconditions.checkArgument(
    isSha256HexString(this.regTxHash),
    "Expect regTxHash to be a hex string representing sha256 hash"
  );
  Preconditions.checkArgument(
    isSha256HexString(this.hashPrevSubTx),
    "Expect hashPrevSubTx to be a hex string representing sha256 hash"
  );
  Preconditions.checkArgument(
    isUnsignedInteger(this.creditFee),
  ...
  Preconditions.checkArgument(
    BufferUtil.isBuffer(this.newPubKey),
    "expect newPubKey to be a Buffer but got " + typeof this.newPubKey
  );
  if (this.payloadSig && this.payloadSigSize !== 0) {
    ...
    Preconditions.checkArgument(
      ...
      "expect payloadSig to be a hex string but got " + typeof this.payloadSig
    );
    Preconditions.checkArgument(
      isUnsignedInteger(this.payloadSigSize),
      "Expect payloadSigSize to be an unsigned integer"
    );
    Preconditions.checkArgument(
      this.payloadSigSize === constants.COMPACT_SIGNATURE_SIZE,
      "Invalid payloadSigSize size"
    );
    Preconditions.checkArgument(
      this.payloadSig.length === constants.COMPACT_SIGNATURE_SIZE * 2,
      "Invalid Argument: Invalid payloadSigSize size"
    );
  }
  return true;
};
 ...
 * @return {SubTxResetKeyPayload}
 */
SubTxResetKeyPayload.prototype.setPubKeyIdFromPrivateKey = function (
  privateKey
) {
  this.setNewPubKeyId(
