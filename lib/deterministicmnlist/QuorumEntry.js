...
QuorumEntry.prototype.verify = function verify(quorumSMNList) {
  ...
  if (!this.isVerified) {
    // verify threshold signature quorumSig

    var quorumPubKey = BLS.PublicKey.fromString(this.quorumPublicKey);



    //signature.verify([hash], [masterPubkey]));
    ...
    if (!aggregatedSignature.verify()) {
      throw new Error('aggregated memberSig is faulty');
    }
