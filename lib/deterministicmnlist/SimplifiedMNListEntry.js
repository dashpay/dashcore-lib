...
SimplifiedMNListEntry.fromBuffer = function fromBuffer(buffer, network) {
  ...
  const rawSimplifiedMNListEntry = {
  ...
  const scriptPayoutSize = bufferReader.readVarintNum();

  if (scriptPayoutSize > 0) {
    rawSimplifiedMNListEntry.payoutAddress = Script(
      bufferReader.read(scriptPayoutSize)
    )
      .toAddress(network)
      .toString();
  }

  const scriptOperatorPayoutAddress = bufferReader.readVarintNum();

  if (scriptOperatorPayoutAddress > 0) {
    rawSimplifiedMNListEntry.operatorPayoutAddress = Script(
      bufferReader.read(scriptPayoutSize)
    )
      .toAddress(network)
      .toString();
  ...
  return SimplifiedMNListEntry.fromObject(rawSimplifiedMNListEntry);
};
 ...
 * @return {Buffer}
 */
SimplifiedMNListEntry.prototype.toBuffer = function toBuffer(extended = false) {
  this.validate();
  ...
  const payoutAddress = this.payoutAddress
    ? Address.fromString(this.payoutAddress)
    : undefined;
  const scriptPayout = new Script(payoutAddress).toBuffer();

  bufferWriter.writeVarintNum(scriptPayout.length);
  bufferWriter.write(scriptPayout);

  const operatorPayoutAddress = this.operatorPayoutAddress
    ? Address.fromString(this.operatorPayoutAddress)
    : undefined;
  const scriptOperatorPayout = new Script(operatorPayoutAddress).toBuffer();

  bufferWriter.writeVarintNum(scriptOperatorPayout.length);
  bufferWriter.write(scriptOperatorPayout);
...
SimplifiedMNListEntry.prototype.validate = function validate() {
  ...
  if (this.payoutAddress) {
    $.checkArgument(
      Address.isValid(this.payoutAddress),
      'payoutAddress is not valid'
    );
  }
  if (this.operatorPayoutAddress) {
    $.checkArgument(
      Address.isValid(this.operatorPayoutAddress),
      'operatorPayoutAddress is not valid'
    );
  }
};
