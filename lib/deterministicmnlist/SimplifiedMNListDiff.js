...
SimplifiedMNListDiff.fromBuffer = function fromBuffer(buffer, network) {
  ...
  for (let i = 0; i < mnListSize; i += 1) {
    data.mnList.push(
      SimplifiedMNListEntry.fromBufferReader(
        bufferReader,
        network,
        { explicitVersion: data.nVersion >= 2 },
      ),
    );
  }
...
SimplifiedMNListDiff.prototype.toBuffer = function toBuffer() {
  ...
  this.mnList.forEach((simplifiedMNListEntry) => {
    bufferWriter.write(simplifiedMNListEntry.toBuffer({ explicitVersion: this.nVersion >= 2 }));
  });
...
const diffHex = require('./diffBlob');

const diff = new SimplifiedMNListDiff(diffObject, 'testnet');

console.assert(diff.toBuffer().toString('hex') === diffHex);

const deserializedDiff = new SimplifiedMNListDiff(diffHex, 'testnet');

console.assert(_.isEqual(diff.toObject(), deserializedDiff.toObject()));
