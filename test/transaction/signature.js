...
describe("TransactionSignature", function () {
  ...
  describe("serialization", function () {
    it("serializes to an object and roundtrips correctly", function () {
      var signature = getSignatureFromTransaction();
      var serialized = signature.toObject();
      expect(new TransactionSignature(serialized).toObject()).to.deep.equal(
        serialized
      );
    });
    ...
    it("can be deserialized with fromObject", function () {
      ...
      var serialized = signature.toObject();
      expect(
        TransactionSignature.fromObject(serialized).toObject()
    ...
    it("can deserialize when signature is a buffer", function () {
      ...
      expect(
        TransactionSignature.fromObject(serialized).toObject()
      ).to.deep.equal(signature.toObject());
    });
    ...
    it("can roundtrip to/from json", function () {
      ...
      expect(TransactionSignature(JSON.parse(json)).toObject()).to.deep.equal(
        serialized
      );
      expect(
        TransactionSignature.fromObject(JSON.parse(json)).toObject()
      ).to.deep.equal(serialized);
    });
    ...
    it("can deserialize a previously known object", function () {
      expect(TransactionSignature(testObject).toObject()).to.deep.equal(
        testObject
      );
    });
  });
