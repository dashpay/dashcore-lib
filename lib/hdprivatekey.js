...
HDPrivateKey.isValidPath = function(arg, hardened) {
  if (_.isString(arg)) {
    ...
    return indexes !== null && _.every(indexes.map(index=>index.index), HDPrivateKey.isValidPath);
  }
...
HDPrivateKey._getDerivationIndexes = function (path) {
  ...
  var indexes = steps.slice(1).map(function (step) {
    ...
    if (!step || step[0] === '-') {
      return {index: NaN, hardened: undefined};
    }

    var nIndex
    // DIP 14 256bit Mode
    if (HDPrivateKey._isIndex256bit(step)) {
      nIndex = HDPrivateKey._IndexArgToBN(step)
      return { index: nIndex, hardened: isHardened }
    }
    // BIP32 Compatibility Mode
    else {
      var index = HDPrivateKey._IndexArgToNumber(step, {castStringToNumber: true})
      if (isHardened) {
        index += HDPrivateKey.Hardened;
      }

      return {index: index, hardened: null} // index is already hardened according to BIP32
    }    
  });

  return _.some(indexes.map(index=>index.index), isNaN) ? null : indexes;
};
...
HDPrivateKey.prototype._deriveFromString = function(path, nonCompliant) {
  ...
  var derived = indexes.reduce(function(prev, index) {
    return _.isNumber(index.index) ? prev._deriveWithNumber(index.index, null, nonCompliant) : prev._deriveWithBigNumber(index.index, index.hardened)
...
HDPrivateKey._isIndex256bit = function (index) {
  if (_.isBuffer(index) && index.length === HDPrivateKey.ChildIndexSize256bit) {
    return true
  }
...
HDPrivateKey.prototype._buildFromSerialized = function(arg) { 
  ...
  if (decoded.length === HDPrivateKey.DataLength) {
    ...
    buffers = {
      ...
      parentFingerPrint: decoded.slice(HDPrivateKey.ParentFingerPrintStart,
        HDPrivateKey.ParentFingerPrintEnd),
      hardened: decoded.slice(HDPrivateKey.HardenedStart, HDPrivateKey.HardenedEnd),
      childIndex: decoded.slice(HDPrivateKey.ChildIndexStart256bit, HDPrivateKey.ChildIndexEnd256bit),
      chainCode: decoded.slice(HDPrivateKey.ChainCodeStart256bit, HDPrivateKey.ChainCodeEnd256bit),
      privateKey: decoded.slice(HDPrivateKey.PrivateKeyStart256bit, HDPrivateKey.PrivateKeyEnd256bit),
      checksum: decoded.slice(HDPrivateKey.ChecksumStart256bit, HDPrivateKey.ChecksumEnd256bit),
      xprivkey: arg
    };
  }
  return this._buildFromBuffers(buffers);
...
HDPrivateKey.prototype._buildFromBuffers = function (arg) {
  ...
  if (arg.childIndex.length === HDPrivateKey.ChildIndexSize256bit) {
    sequence = [
      arg.version, arg.depth, arg.parentFingerPrint, arg.hardened, arg.childIndex, arg.chainCode,
