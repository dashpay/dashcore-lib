...
const bls = {
    ...
    async verifySignature(signatureHex, messageHash, publicKeyHex) {
        ...
        try {
            thresholdSignature = BLS.Signature.fromBytes(
                Uint8Array.from(Buffer.from(signatureHex, 'hex'))
            )
            publicKey = BLS.PublicKey.fromBytes(
                Uint8Array.from(Buffer.from(publicKeyHex, 'hex'))
            )
            aggregationInfo = BLS.AggregationInfo.fromMsgHash(
                publicKey,
                messageHash
            )
    ...
    async aggregatePublicKey(publicKeys, signersBits) {
        ...
        const aggregatedKeyHex = Buffer.from(
            aggregatedKey.serialize()
        ).toString('hex')
