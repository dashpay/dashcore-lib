...
SimplifiedMNList.prototype.applyDiff = function applyDiff(simplifiedMNListDiff) {
  ...
  this.addOrUpdateQuorums(diff.newQuorums);
  ...
  this.validQuorums = this.quorumList.filter(function (quorumEntry) {
    return quorumEntry.isValid;
  });
};
 ...
 */
SimplifiedMNList.prototype.addOrUpdateQuorums = function addMNs(quorumEntries) {
  var newQuorumEntries = quorumEntries.map(function (quorumEntry) {
    return quorumEntry.copy();
  });
  newQuorumEntries.forEach(function (newQuorumListEntry) {
    var indexOfOldEntry = this.quorumList.findIndex(function (oldQuorumListEntry) {
      return oldQuorumListEntry.proRegTxHash === newQuorumListEntry.proRegTxHash;
    });
    if (indexOfOldEntry > -1) {
      this.quorumList[indexOfOldEntry] = newQuorumListEntry;
    } else {
      return this.quorumList.push(newQuorumListEntry);
    }
  }, this);
};
 ...
 * @param {string[]} quorumHashes - list of quorumHashes to delete from quorum list
 */
SimplifiedMNList.prototype.deleteQuorums = function deleteQuorums(quorumHashes) {
  quorumHashes.forEach(function (quorumHash) {
    var quorumIndex = this.quorumList.findIndex(function (quorum) {
      return quorum.quorumHash === quorumHash;
    });
    if (quorumIndex > -1) {
...
/**
 * @private
 * Sorts quorum list in deterministic order
 */
SimplifiedMNList.prototype.sortQuorums = function sortQuorums() {
  this.quorumList.sort(function (a, b) {
    return Buffer.compare(Buffer.from(a.quorumHash, 'hex').reverse(), Buffer.from(b.quorumHash, 'hex').reverse());
  });
};
...
SimplifiedMNList.prototype.calculateMerkleRootQuorums = function calculateMerkleRootQuorums() {
  ...
  this.sortQuorums();
  var sortedQuorumHashes = this.quorumList.map(
    function (quorumEntry) {
      return quorumEntry.calculateHash();
    }
  );
  return getMerkleRoot(getMerkleTree(sortedQuorumHashes)).reverse().toString('hex');
};
 ...
 * @returns {SimplifiedMNListEntry[]}
 */
SimplifiedMNList.prototype.getValidQuorumList = function getValidQuorumList() {
  return this.validQuorums;
};
