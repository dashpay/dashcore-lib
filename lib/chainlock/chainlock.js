...
const { isHexString, isUnsignedInteger, isHexStringOfSize } = require('../util/js');

const { SHA256_HASH_SIZE, BLS_SIGNATURE_SIZE } = constants;
const getBLSInstance = require('../crypto/bls').getInstance;
...
ChainLock.prototype.verifySignatureAgainstQuorum = async function verifySignatureAgainstQuorum(quorumEntry) {
  ...
  const blsInstance = await getBLSInstance();
