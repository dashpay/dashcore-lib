...
SimplifiedMNListEntry.fromBuffer = function fromBuffer(buffer, network) {
  ...
  return SimplifiedMNListEntry.fromObject({
    ...
    // nVersion: Number(bufferReader.readUInt8()),
    nType: Number(bufferReader.readUInt8()),
    platformHTTPPort: Number(bufferReader.readUInt8()),
  });
};
...
SimplifiedMNListEntry.prototype.toBuffer = function toBuffer() {
  ...
  // bufferWriter.writeUInt8(Number(this.nVersion));
  bufferWriter.writeUInt8(Number(this.nType));
  bufferWriter.writeUInt8(Number(this.platformHTTPPort));
