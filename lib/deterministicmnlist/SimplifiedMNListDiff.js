 ...
 */
function SimplifiedMNListDiff(arg, network, llmqOverride = {}) {
  if (arg) {
    ...
    if (BufferUtil.isBuffer(arg)) {
      return SimplifiedMNListDiff.fromBuffer(arg, validNetwork, llmqOverride);
    }
    if (_.isObject(arg)) {
      return SimplifiedMNListDiff.fromObject(arg, validNetwork, llmqOverride);
    }
    if (isHexString(arg)) {
      return SimplifiedMNListDiff.fromHexString(arg, validNetwork, llmqOverride);
    }
    throw new TypeError(
 ...
 */
SimplifiedMNListDiff.fromBuffer = function fromBuffer(buffer, network, llmqOverride = {}) {
  ...
  for (let i = 0; i < newQuorumsSize; i += 1) {
    ...
    data.newQuorums.push(QuorumEntry.fromBuffer(entry, llmqOverride));
  }
  ...
  return this.fromObject(data, network, llmqOverride);
};
 ...
 * @return {SimplifiedMNListDiff}
 */
SimplifiedMNListDiff.fromObject = function fromObject(obj, network, llmqOverride = {}) {
  ...
  simplifiedMNListDiff.newQuorums = obj.newQuorums.map(
    (quorumEntry) => new QuorumEntry(quorumEntry, llmqOverride)
  );
