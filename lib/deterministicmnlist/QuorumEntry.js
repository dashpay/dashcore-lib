...
QuorumEntry.prototype.isValidQuorumSig = function isValidQuorumSig() {
  ...
  var quorumPubKey = BLS.PublicKey.fromBytes(Uint8Array.from(Buffer.from(this.quorumPublicKey, 'hex')));
  var aggregationInfo = BLS.AggregationInfo.fromMsgHash(quorumPubKey, msgHash);
  var thresholdSignature = BLS.Signature.fromBytes(Uint8Array.from(Buffer.from(this.quorumSig, 'hex')));
 ...
 * @param {SimplifiedMNList} mnList
 * @return {boolean}
 */
QuorumEntry.prototype.isValidMemberSig = function isValidMemberSig(mnList) {
  // verify aggregated signature memberSig
  var quorumMembers = this.getAllQuorumMembers(mnList);
  var aggregatedSignature = BLS.Signature.fromString(this.membersSig);
  var pks = [];
  var i = 0;

  // aggregated all pubKeyOperators of signing members
  quorumMembers.forEach((member) => {
    //this.validMembers[i]
    pks.push(Buffer.from(member.pubKeyOperator, 'hex'));
    i += 1;
  });

  var aggregatedPubKey = PublicKey.aggregate(pks);
  var aggregationInfo = BLS.AggregationInfo.fromMsg(aggregatedPubKey, message);
  aggregatedSignature.setAggregationInfo(aggregationInfo);

  return aggregatedSignature.verify(msgHash, quorumPubKey);
};
...
QuorumEntry.prototype.verify = function verify(quorumSMNList) {
  ...
  if (!this.isVerified) {
    return (this.isValidQuorumSig && this.isValidMemberSig)
  }
  return true;
};

/**
 * Get all members for this quorum both valid and invalid
 * @param {SimplifiedMNList} SMNList - MNlist for the block the quorum was starting
 * its DKG session with
...
QuorumEntry.prototype.getAllQuorumMembers = function getAllQuorumMembers(SMNList) {
  ...
  return SMNList.calculateQuorum(this.getSelectionModifier());
};
