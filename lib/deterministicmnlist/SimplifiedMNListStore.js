...
function SimplifiedMNListStore(diffArray, options) {
  ...
  this.maxDiffs =
    typeof options !== "undefined" && options.maxListsLimit
      ? options.maxListsLimit
      : SMLSTORE_MAX_DIFFS;
  this.diffStore = [];
 ...
 * @param {string} hash
 */
SimplifiedMNListStore.prototype.updateTipInfo = function updateTipInfo(
  height,
  hash
) {
 ...
 */
SimplifiedMNListStore.prototype.getSMLDiffbyHeightRange =
  function getSMLDiffbyHeightRange(startHeight, endHeight) {
    return this.diffStore
      .filter((diff) => diff.height >= startHeight && diff.height <= endHeight)
      .map((filteredDiffs) => filteredDiffs.diff);
  };
 ...
 */
SimplifiedMNListStore.prototype.getSMLbyHeight = function getSMLbyHeight(
  height
) {
 ...
 * @returns {SimplifiedMNList}
 */
SimplifiedMNListStore.prototype.createListFromBaseAndDiffs =
  function createListFromBaseAndDiffs(diffs) {
    const createdList = new SimplifiedMNList(
      this.baseSimplifiedMNList.toSimplifiedMNListDiff(),
      this.network
    );
    diffs.forEach((diff) => {
...
SimplifiedMNListStore.fromJSON = function fromJSON(smlStoreJSON) {
  const baseDiff = SimplifiedMNList.fromJSON(
    smlStoreJSON.baseSimplifiedMNList
  ).toSimplifiedMNListDiff();
  // Getting jsons of all other diffs
  const diffsFromFile = smlStoreJSON.diffStore.map((diff) =>
    SimplifiedMNListDiff.fromJSON(diff.diff)
  );
