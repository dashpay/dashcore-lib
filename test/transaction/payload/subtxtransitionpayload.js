...
describe('SubTxTransitionPayload', function () {
    ...
    describe('#hashSTPacket', function () {
        it('Should set hashSTPacket and return instance back', function () {
            ...
            var payload = new SubTxTransitionPayload().setHashSTPacket(
                hashSTPacket
            )
    ...
    describe('fromJSON', function () {
        it('Should return instance of SubTxTransitionPayload with correct parsed data', function () {
            ...
            expect(payload.hashPrevSubTx).to.be.deep.equal(
                payloadJSON.hashPrevSubTx
            )
            expect(payload.hashSTPacket).to.be.deep.equal(
                payloadJSON.hashSTPacket
            )
        ...
        it('Should throw an error if the data is incorrect', function () {
            ...
            var payloadsWithInvalidVersions = invalidVersions.map(function (
                version
            ) {
            ...
            payloadsWithInvalidVersions.forEach(function (
            ...
            expect(function () {
                SubTxTransitionPayload.fromJSON(
                    payloadWithIncorrectHashSTPacketSize
                )
            ...
            expect(function () {
                SubTxTransitionPayload.fromJSON(
                    payloadWithIncorrectHashPrevSubTx
                )
            }).to.throw('Invalid Argument: Invalid hashPrevSubTx size')
            expect(function () {
                SubTxTransitionPayload.fromJSON(payloadWithIncorrectSignature)
            }).to.throw(
                'Invalid Argument: expect payloadSig to be a hex string'
            )
            expect(function () {
                SubTxTransitionPayload.fromJSON(
                    payloadWithIncorrectSignatureSize
                )
    ...
    describe('#sign', function () {
        ...
        it('Should sign payload and return instance back if a private key is an instance of PrivateKey', function () {
            var payload = validPayloadFixture
                .copy()
    ...
    describe('#toJSON', function () {
        it('Should return a JSON that contains same data as the payload instance', function () {
            ...
            expect(payloadJSON.hashPrevSubTx).to.be.deep.equal(
                payload.hashPrevSubTx
            )
            expect(payloadJSON.hashSTPacket).to.be.deep.equal(
                payload.hashSTPacket
            )
