...
PartialMerkleTree.build = function build(transactionHashes, filterMatches) {
  ...
  partialTree.merkleFlags = convertBitArrayToUInt8Array(partialTree.internalFlags.reverse());
  return partialTree;
};
...
PartialMerkleTree.prototype.traverseAndBuild = function traverseAndBuild(height, pos, txIds, matches) {
  ...
  // store as flag bit
  this.internalFlags.push(Boolean(parentOfMatch));
  if (height === 0 || !Boolean(parentOfMatch)) {
    // this.internalFlags.push(Boolean(parentOfMatch));
    // if at height 0, or nothing interesting below, store hash and stop
    this.merkleHashes.push(calculateHashAtHeight(height, pos, txIds).toString('hex'));
  } else {
