...
async function verify(SMLStore) {
  const signatoryQuorum = this.selectSignatoryQuorum(SMLStore, this.getRequestId(), this.blockHash);
  return this.verifySignatureAgainstQuorum(signatoryQuorum);
}
 ...
 * @param {Buffer} selectionHash
 * @returns {QuorumEntry} - signatoryQuorum
 */
ChainLock.prototype.selectSignatoryQuorum = function selectSignatoryQuorum(SMLStore, requestId, selectionHash) {
  const chainlockSML = SMLStore.getSMLbyHeight(this.height);
  const scoredQuorums = chainlockSML.calculateSignatoryQuorumScores(
    chainlockSML.getChainlockLLMQType(), selectionHash,
  );

  scoredQuorums.sort((a, b) => Buffer.compare(a.score, b.score));
  return scoredQuorums[scoredQuorums[0].index].quorum;
};
