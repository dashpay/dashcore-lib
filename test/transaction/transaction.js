...
describe('Transaction', function () {
    ...
    it('should autofill version field if nothing passed to constructor', function () {
        ...
        var serialized = transaction
            .sign(new PrivateKey(testKey))
            .serialize(true)
        expect(new Transaction(serialized).version).to.be.equal(
    ...
    var anyoneCanSpendUTXO = JSON.parse(
    ...
    describe('to and from JSON', function () {
        it('takes a string that is a valid JSON and deserializes from it', function () {
            ...
            expect(
                new Transaction(simple.toJSON()).uncheckedSerialize()
            ).to.equal(simple.uncheckedSerialize())
            ...
            expect(ctx.uncheckedSerialize()).to.equal(
                complex.uncheckedSerialize()
            )
        })
    ...
    describe('serialization of inputs', function () {
        ...
        it('can serialize and deserialize a P2SH input', function () {
            ...
            expect(
                deserialized.inputs[0] instanceof
                    Transaction.Input.MultiSigScriptHash
            ).to.equal(true)
        })
    })
    ...
    describe('handling the nLockTime', function () {
        ...
        it('accepts a number instance with a timestamp', function () {
            ...
            transaction
                .getLockTime()
                .should.deep.equal(new Date(timestamp * 1000))
        })
        it('accepts a block height', function () {
            var transaction = new Transaction().lockUntilBlockHeight(
                blockHeight
            )
    ...
    it('handles unsupported utxo in tx object', function () {
        ...
        transaction.fromObject
            ...
            .should.throw(
                'Unsupported input script type: OP_1 OP_ADD OP_2 OP_EQUAL'
            )
    })
        ...
        describe('bitcoinjs fixtures', function () {
            ...
            fixture.outputs.forEach(function (outputSet) {
                it(outputSet.description, function () {
                    var tx = new Transaction()
                    outputSet.outputs = outputSet.outputs.map(function (
                        output
                    ) {
                        return new Output({
                    ...
                    getIndexOrder(
                        outputSet.outputs,
                        tx.outputs
                    ).should.deep.equal(outputSet.expected)
                })
            })
        })
    })
    ...
    describe('setExtraPayload', function () {
        ...
        it('Should set payload and size', function () {
            ...
            var expectedPayloadSize =
                2 + 1 + nameSize + Payload.constants.PUBKEY_ID_SIZE + 1
        ...
        it('Should throw when trying to serialize special transaction without any payload', function () {
            ...
            expect(function () {
                transaction.sign(privateKey).serialize()
            }).to.throw('Transaction payload size is invalid')
        })
        it('Should throw when extra payload is set, but special transaction type is not set', function () {
            ...
            }).to.throw('Special transaction type is not set')
        })
    })
    describe('isSpecialTransaction', function () {
        it('Should return true if a transaction is qualified to be a special transaction', function () {
            var transaction = Transaction().setType(
                Transaction.TYPES.TRANSACTION_COINBASE
            )
    ...
    describe('setType', function () {
        it('Should set type and create payload', function () {
            var transaction = new Transaction().setType(
                Transaction.TYPES.TRANSACTION_SUBTX_REGISTER
            )
        ...
        it('Should throw an error if second param is not a hash', function () {
            expect(function () {
                new Transaction()
                    ...
                    .to(
                        'yT9Lms2ATYLd3QLA4pVpg3mQ5KiHB9Dp1b',
                        34518076547 - 11000
                    )
    ...
    describe('getOutPointBuffer', function () {
        it('Should return an outpoint buffer', function () {
            var publicKeyHash = 'b8e2d839dd21088b78bebfea3e3e632181197982'
            var txHash =
                'fbf921b1431c7916f2089999a6c1006c35edfd3f746eaa8da41b014bef754a7a'
    ...
    describe('parseOutPointBuffer', function () {
        it('Should return transaction hash and the output index', function () {
            ...
            expect(parsedOutPoint.transactionHash).to.be.equal(
                'fbf921b1431c7916f2089999a6c1006c35edfd3f746eaa8da41b014bef754a7a'
            )
    ...
    describe('Special transaction payload integration', function () {
        ...
        describe('Registration transaction', function () {
            it('Should parse special transaction correctly', function () {
                ...
                expect(parsedTransaction.type).to.be.equal(
                    Transaction.TYPES.TRANSACTION_SUBTX_REGISTER
                )
                expect(parsedTransaction.extraPayload.version).to.be.equal(1)
                expect(parsedTransaction.extraPayload.userName).to.be.equal(
                    username
                )
                expect(
                    parsedTransaction.extraPayload.pubKeyId.toString('hex')
                ).to.be.equal(expectedPubKeyId)

                expect(
                    parsedTransaction.extraPayload.verifySignature(
                        expectedPubKeyId
                    )
                ).to.be.true
                expect(
            ...
            it('Should create valid hex', function () {
                ...
                expect(
                    transaction.extraPayload.pubKeyId.toString('hex')
                ).to.be.equal(expectedPubKeyId)

                expect(
                    transaction.extraPayload.verifySignature(expectedPubKeyId)
                ).to.be.true
                expect(transaction.extraPayload.verifySignature(randomPubKeyId))
                    .to.be.false
            })
        })
        ...
        describe('Provider Register Transaction with collateral (protx register)', function () {
            ...
            it('Should parse the payload if transaction serialized as a JSON ', function () {
                var tx = new Transaction(
                    proRegTxFixture.getProRegTransactionJSON()
                )
            ...
            it('Should serialize transaction to the same string', function () {
                ...
                expect(tx.serialize(true)).to.be.equal(
                    proRegTxFixture.getProRegTransactionHex()
                )
            })
        })
        ...
        describe('Provider Update Registrar Transaction', function () {
            it('Should parse the payload if transaction serialized as a hex string', function () {
                var tx = new Transaction(
                    proUpRegTxFixture.getProUpRegTransactionHex()
                )
            ...
            it('Should parse the payload if transaction serialized as a JSON ', function () {
                var tx = new Transaction(
                    proUpRegTxFixture.getProUpRegTransactionJSON()
                )
        ...
        describe('Provider Update Revoke Transaction', function () {
            it('Should parse the payload if transaction serialized as a hex string', function () {
                var tx = new Transaction(
                    proUpRevTxFixture.getProUpRevTransactionHex()
                )
            ...
            it('Should parse the payload if transaction serialized as a JSON ', function () {
                var tx = new Transaction(
                    proUpRevTxFixture.getProUpRevTransactionJSON()
                )
        ...
        describe('Provider Service Update Transaction ', function () {
            it('Should parse the payload if transaction serialized as a hex string', function () {
                var tx = new Transaction(
                    proUpServFixture.getProUpServTransactionHex()
                )
            ...
            it('Should parse the payload if transaction serialized as a JSON ', function () {
                var tx = new Transaction(
                    proUpServFixture.getProUpServTransactionJSON()
                )
        ...
        describe('Quorum Commitment Transaction ', function () {
            it('Should parse the payload', function () {
                var transactionHex =
                    ''
        ...
        describe('State transition', function () {
            ...
            it('Should parse and verify hex', function () {
                ...
                expect(transaction.extraPayload.hashSTPacket).to.be.equal(
                    hashSTPacket
                )
                expect(transaction.extraPayload.creditFee).to.be.equal(
                    creditFee
                )
