...
function traverseAndBuildPartialTree(height, position, hashes, matches) {
  ...
  if (height === 0 || !Boolean(parentOfMatch)) {
    ...
    var res = traverseAndBuildPartialTree(height - 1, position * 2, hashes, matches);
    flags = flags.concat(res.flags);
    merkleHashes = merkleHashes.concat(res.merkleHashes);
    if (position * 2 + 1 < calculateTreeWidth(hashes.length,height - 1)) {
      res = traverseAndBuildPartialTree(height - 1, position * 2 + 1, hashes, matches);
      flags = flags.concat(res.flags);
      merkleHashes = merkleHashes.concat(res.merkleHashes);
    }
  }
