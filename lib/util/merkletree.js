 ...
 * @return {Buffer}
 */
function calculateHashAtHeight(totalElementsCount, height, position, hashes) {
  if (height === 0) {
    return hashes[position];
  } else {
    // calculate left hash
    var left = calculateHashAtHeight(totalElementsCount,height - 1, position * 2, hashes);
    var right = left;
    // calculate right hash if not beyond the end of the array - copy left hash otherwise
    if (position * 2 + 1 < calculateTreeWidth(totalElementsCount, height - 1)) {
      right = calculateHashAtHeight(totalElementsCount,height - 1, position * 2 + 1, hashes);
    }
