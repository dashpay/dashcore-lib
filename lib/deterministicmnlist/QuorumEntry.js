...
QuorumEntry.prototype.isValidMemberSig = function isValidMemberSig(mnList) {
  ...
  var aggregatedSignature = BLS.Signature.fromBytes(Uint8Array.from(Buffer.from(this.membersSig, 'hex')));
  var pks = [];

  var signersArray = Uint8Array.from(Buffer.from(this.signers, 'hex'));
  var signersBits = BitArray.uint8ArrayToBitArray(signersArray);

  // aggregate all pubKeyOperators of signing members
  var i = 0;
  quorumMembers.forEach((member) => {
    if(signersBits[i]) {
      pks.push(BLS.PublicKey.fromBytes(Uint8Array.from(Buffer.from(member.pubKeyOperator, 'hex'))));
    }
    i += 1;
  });

  var aggregatedPubKey = BLS.PublicKey.aggregate(pks);
  var msgHash = Uint8Array.from(this.getCommitmentHash());
  var aggregationInfo = BLS.AggregationInfo.fromMsgHash(aggregatedPubKey, msgHash);
  aggregatedSignature.setAggregationInfo(aggregationInfo);

  return aggregatedSignature.verify();
};
