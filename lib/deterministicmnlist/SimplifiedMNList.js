 ...
 * @returns {boolean}
 */
SimplifiedMNList.prototype.verifyQuorums = function verifyQuorums() {
...
/**
 * Returns all already of verified quorums
 * @returns {QuorumEntry[]}
 */
SimplifiedMNList.prototype.getVerifiedQuorums = function getVerifiedQuorums() {
  return this.quorumList.map(function(quorum) {
    if (quorum.isVerified) {
      return quorum;
    }
  });
};
 ...
 * The first part is already pre-calculated (confirmedHashWithProRegTxHash)
...
SimplifiedMNList.prototype.calculateScores = function calculateScores(modifier) {
  ...
  scores = [];
  var allMns = this.getValidMasternodesList().slice();
  allMns.forEach(function(mn) {
    if(!mn.confirmedHash) {
      // we only take confirmed MNs into account
      return;
    }
    ...
    var score = Hash.sha256(bufferWriter.toBuffer()).reverse();
    scores.push({ score: score, mn: mn });
  });
  return scores;
};
